...
void ManipulationShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* heightMap, ID3D11ShaderResourceView* normalsMap, Light* light, float& time)
{
	HRESULT result;
	...
	timePtr->time = time;
	timePtr->padding = XMFLOAT3(0.0f, 0.0, 0.0f);
	...
	deviceContext->PSSetShaderResources(1, 1, &normalsMap);
